...
def apply_canonicalizer_v2_numpy(
    ...
    # Canonicalizer V2 contract: grayscale 2D uint8 input.
    # Convert only the input format here; the canonicalization algorithm
    # itself remains unchanged/frozen.
    arr = np.asarray(
        image.convert("L"),
        dtype=np.uint8,
    )
    ...
    if arr.ndim != 2:
        raise ValueError(
            f"Expected grayscale 2D input for Canonicalizer V2, got {arr.shape}"
        )
