...
def build_snapshot(
    ...
    # Evita review_notes_x / review_notes_y no merge.
    state_columns = [
        "source_id",
        "review_decision",
        "review_notes",
    ...
    state_merge = state[state_columns].copy()

    # Remove do master eventuais colunas que pertencem ao state.
    # Isso evita colisão de nomes durante o merge.
    master = master.drop(
        columns=[
            col
            for col in ["review_decision", "review_notes"]
